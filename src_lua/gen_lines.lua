...
function gen_line1(flnm_energy, flnm_energy2, flnm_wait, verbose)
...
--t_first= 15522.682034838
   if 1==20 and filename_wait_txt ~= "" then
	   ...
	   if ts0 == -1.0 then
		  error("didn't find 't_start= xxx' string in wait txt file: '"..filename_wait_txt.."'. bye.")
	   end
   else
	   ts0 = 0
   end
   ...
   col = {["ts"] = "_TIMESTAMP_", ["dura"] = "_DURATION_", ["line"] = "line", ["area"] = "area", ["event"]= "event"}
   local rows = 0
   local k, v, evt
   -- 'gen_line1' must match event_name field in charts.json
   local evt_str = "gen_line1"
   evt_hash[evt_str] = 0
   local dff = 0.5
   for k = 0, 20, 1
   do
	 ...
	 if  k == 10 then
	 	...
	 	--t1[col.dura] = 1
	 	t1[col.line] =  2*k - 4
     	t1[col.event] = evt_str
     	t1[col.area]  = "test"
     	evt_hash[evt_str] = evt_hash[evt_str] + 1
	 	rows = rows + 1
     	table.insert(data_table, t1)

		t1 = {}
	 	t1[col.ts] = k
	 	t1[col.dura] = 1 - dff - .01
	 	--t1[col.dura] = 1
	 	t1[col.line] =  2*k -3
     	t1[col.event] = evt_str
     	t1[col.area]  = "test"
