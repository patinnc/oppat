...
function L2_mpki(verbose)
	--print "in lua routine L2_mpki"
	local ts0
	...
	if type(new_cols_hash) ~= "table" then
		...
		local tbl_ck_new_cols = {"L2_mpki", "__EMIT__", "duration", "area"}
	...
	if tbl[cpu][4] > 0 and tbl[cpu][4] == tbl[cpu][5] then
		local L2_mpki_idx  = new_cols_hash["L2_mpki"];
		local area_idx = new_cols_hash["area"];
		local dura_idx = new_cols_hash["duration"];
		local inst   = tbl[cpu][2]
		local misses = tbl[cpu][3]
		...
		if dura > 0.0 and inst > 0.0 then
			local val = misses / (inst/1000.0)
			--if ld_stl > 120.0 then
			--	ld_stl = 120.0
			--end
			new_vals[L2_mpki_idx] = val
