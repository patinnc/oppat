#!/bin/bash
ph_end=Single-Core PDF Rendering
ph_end=Single-Core LZMA
ph_end=Multi-Core Memory Bandwidth
rm pat*.png
make && bin/oppat.x -r ../oppat_data/lnx/gb8 --cpu_diagram web/haswell_block_diagram.svg --ph_image pat --by_phase --phase0 "Single-Core AES" --phase1 "$(ph_end)" --img_wxh_pxls 1017,1388 > tmp.jnk
